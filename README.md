...
11. I have a [WaveShare Rp2040-Zero](https://www.waveshare.com/rp2040-zero.htm) device that look like promising alternatives to the SSMicro as an  MCU for keyboard hacks. Faster dual cores could be interesting; perhaps one running a decoding thread while the other core scans the matrix?. At present I don't think the speed of the code execution is the biggest issue (the limiting factor is more the speed of the decade counters & shift register, and the debounce delay for the switches).
